...
def test_graph_from_file_local_file(model_name):
    ...
    shutil.copyfile(
        "/workspaces/llm-graph-builder/backend/files/About Amazon.pdf",
    ...
    try:
        assert (
            local_file_result["status"] == "Completed"
            and local_file_result["nodeCount"] > 0
            and local_file_result["relationshipCount"] > 0
        )
...
def test_graph_from_file_local_file_failed(model_name):
    ...
    try:
        ...
        local_file_result = extract_graph_from_file_local_file(
            graph, model, file_name, merged_file_path, "", ""
        )

        print(local_file_result)
    except AssertionError as e:
        print(
            "Failed due to file does not exist means not uploaded or accidentaly deleteled from server"
        )
...
def test_graph_from_Wikipedia(model_name):
    ...
    wikiresult = extract_graph_from_file_Wikipedia(
        uri, userName, password, database, model, file_name, 1, "en", "", ""
    )
    ...
    try:
        assert (
            wikiresult["status"] == "Completed"
            and wikiresult["nodeCount"] > 0
            and wikiresult["relationshipCount"] > 0
        )
        return wikiresult
        print("Success")
    except AssertionError as e:
...
def test_graph_from_youtube_video(model_name):
    ...
    youtuberesult = extract_graph_from_file_youtube(
        uri, userName, password, database, model, source_url, "", ""
    )
    ...
    try:
        assert (
            youtuberesult["status"] == "Completed"
            and youtuberesult["nodeCount"] > 1
            and youtuberesult["relationshipCount"] > 1
        )
...
def test_graph_from_file_test_gcs():
    ...
    create_source_node_graph_url_gcs(
        graph, model, bucket_name, folder_name, source_type
    )
    gcsresult = extract_graph_from_file_gcs(
        graph, model, bucket_name, folder_name, file_name, "", ""
    )
    ...
    try:
        assert (
            gcsresult["status"] == "Completed"
            and gcsresult["nodeCount"] > 10
            and gcsresult["relationshipCount"] > 5
        )
        print("Success")
    except AssertionError as e:
        print("Failed ", e)
...
if __name__ == "__main__":
    ...
    df.to_csv(
        f"Integration_TestResult_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        index=False,
    )
