...
class ParallelComponent:
    ...
    async def _cypher_results(self):
        try:
            ...
            error_message = str(e)
            logging.exception(
                f"Exception in CypherQAChain in QA component:{error_message}"
            )
            #   raise Exception(error_message)
...
async def main(uri, userName, password, question, session_id):
    ...
    parallel_component = ParallelComponent(
        uri, userName, password, question, session_id
    )
    ...
    ai_message, user_message = await parallel_component.final_prompt(
        f_chat_summary, f_cypher_result, f_vector_result
    )
