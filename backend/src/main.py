    ...
    if result[0]['Status'] != 'Processing':      
      ...
      for i in range(0, len(chunkId_chunkDoc_list), update_graph_chunk_processed):
        ...
        if bool(is_cancelled_status) == True:
          ...
          node_count,rel_count = await processing_chunks(selected_chunks,graph,uri, userName, password, database,file_name,model,allowedNodes,allowedRelationship,node_count, rel_count)
