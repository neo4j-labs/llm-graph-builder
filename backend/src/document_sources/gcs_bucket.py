...
from langchain_community.document_loaders import (GCSDirectoryLoader,
                                                  GCSFileLoader, PyMuPDFLoader)
from langchain_core.documents import Document
from PyPDF2 import PdfReader

from .local_file import get_pages_with_page_numbers, load_document_content


def get_gcs_bucket_files_info(
    gcs_project_id, gcs_bucket_name, gcs_bucket_folder, creds
):
    ...
    try:
        ...
        if bucket.name in buckets_list:
            blobs = storage_client.list_blobs(
                ...
                prefix=gcs_bucket_folder if gcs_bucket_folder else "",
            )
            lst_file_metadata = []
            for blob in blobs:
            ...
            logging.info(
                f"Bucket : {gcs_bucket_name} does not exist in project : {gcs_project_id}"
            )
...
def get_documents_from_gcs(
    ...
    if access_token is None:
        ...
        loader = GCSFileLoader(
            ...
            blob=blob_name,
            loader_func=load_document_content,
        )
        pages = loader.load()
...
def upload_file_to_gcs(
    file_chunk, chunk_number, original_file_name, bucket_name, folder_name_sha1_hashed
):
    try:
...
def merge_file_gcs(
    bucket_name, original_file_name: str, folder_name_sha1_hashed, total_chunks
):
    try:
        storage_client = storage.Client()
        bucket = storage_client.bucket(bucket_name)
        ...
        file_name_with__hashed_folder = (
            folder_name_sha1_hashed + "/" + original_file_name
        )
