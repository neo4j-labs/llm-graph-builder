...
def get_calculated_timestamps(chunks, youtube_id):
    ...
    for chunk in chunks:
        ...
        chunk.metadata["start_timestamp"] = str(timedelta(seconds=start_time)).split(
            "."
        )[0]
...
def get_chunks_with_timestamps(chunks):
    ...
    for chunk in chunks:
        chunk.metadata["end_timestamp"] = str(
            timedelta(seconds=chunk.metadata["start_seconds"] + 60)
        ).split(".")[0]
    return chunks
