...
def setup_chat(model, graph, document_names, chat_mode_settings):
    ...
    try:
        ...
        llm, model_name, callback_handler  = get_llm(model=model)
...
def process_chat_response(messages, history, question, model, graph, document_names, chat_mode_settings,email=""):
    try:
        ...
        if docs:
            ...
            print("HEYYYYYY")
            track_token_usage(email=email,usage=total_tokens,last_used_model=model)
        else:
...
def create_graph_chain(model, graph):
    try:
        ...
        cypher_llm,model_name,callback_handler = get_llm(model)
        qa_llm,model_name,callback_handler = get_llm(model)
...
def QA_RAG(graph,model, question, document_names, session_id, mode, write_access=True,email=""):
    ...
    if mode == CHAT_GRAPH_MODE:
        ...
        if document_names and not chat_mode_settings["document_filter"]:
            ...
            result = process_chat_response(messages,history, question, model, graph, document_names,chat_mode_settings,email)
