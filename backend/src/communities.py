...
def create_community_summaries(gds, model, email):
...
def create_community_properties(gds, model, email):
    commands = [
    ...
    try:
        ...
        create_community_summaries(gds, model, email)
...
def create_communities(uri, username, password, database,email=None,model=COMMUNITY_CREATION_DEFAULT_MODEL):
    try:
        ...
        if write_communities_sucess:
            ...
            create_community_properties(gds,model,email)
