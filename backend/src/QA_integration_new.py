...
def get_neo4j_retriever(
    ...
    try:
        if mode == "hybrid":
            ...
            logging.info(
                f"Successfully retrieved Neo4jVector index '{index_name}' and keyword index '{keyword_index}'"
            )
        else:
            neo_db = Neo4jVector.from_existing_index(
                embedding=EMBEDDING_FUNCTION,
        ...
        if document_names:
            ...
            logging.info(
                f"Successfully created retriever for index '{index_name}' with search_k={search_k}, score_threshold={score_threshold} for documents {document_names}"
            )
        else:
            retriever = neo_db.as_retriever(
                search_type="similarity_score_threshold",
            ...
            logging.info(
                f"Successfully created retriever for index '{index_name}' with search_k={search_k}, score_threshold={score_threshold}"
            )
        return retriever
        ...
        raise Exception(
            "An error occurred while retrieving the Neo4jVector index '{index_name}' or creating the retriever. Please drop and create a new vector index: {e}"
        ) from e
...
def get_sources_and_chunks(sources_used, docs):
    ...
    result = {"sources": sources_used, "chunkdetails": chunkdetails_list}
    return result
...
def create_graph_chain(model, graph):
    try:
        ...
        logging.error(
            f"An error occurred while creating the GraphCypherQAChain instance. : {e}"
        )
...
def QA_RAG(graph, model, question, document_names, session_id, mode):
    try:
        ...
        if mode == "graph":
            ...
            ai_response = (
                AIMessage(content=graph_response["response"])
                if graph_response["response"]
                else AIMessage(content="Something went wrong")
            )
        ...
        llm, doc_retriever, model_version = setup_chat(
            model, graph, document_names, retrieval_query, mode
        )

        docs = retrieve_documents(doc_retriever, messages)

        if docs:
            content, result, total_tokens = process_documents(
                docs, question, messages, llm, model
            )
        else:
