...
async def chunk_entities(uri=Form(),userName=Form(), password=Form(), database=Form(), chunk_ids=Form(None),type=Form()):
    try:
        logging.info(f"URI: {uri}, Username: {userName}, chunk_ids: {chunk_ids}")
        result = await asyncio.to_thread(get_entities_from_chunkids,uri=uri, username=userName, password=password, database=database,node_ids=chunk_ids,type=type)
