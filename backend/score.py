...
from src.post_processing import create_entity_embedding, create_fulltext
    ...
    try:
        ...
        if source_type == "local file":
            if gcs_file_cache == "True":
                ...
                copy_failed_file(
                    BUCKET_UPLOAD, BUCKET_FAILED_FILE, folder_name, file_name
                )
                time.sleep(5)
...
async def post_processing(
    uri=Form(), userName=Form(), password=Form(), database=Form(), tasks=Form(None)
):
    try:
        graph = create_graph_database_connection(uri, userName, password, database)
        ...
        if "create_fulltext_index" in tasks:
            ...
            josn_obj = {
                "api_name": "post_processing/create_fulltext_index",
                "db_url": uri,
                "logging_time": formatted_time(datetime.now(timezone.utc)),
            }
            logger.log_struct(josn_obj)
        ...
        if (
            ...
            and "create_entity_embedding" in tasks
        ):
...
async def chunk_entities(
    uri=Form(), userName=Form(), password=Form(), chunk_ids=Form(None)
):
    try:
...
async def clear_chat_bot(
    uri=Form(), userName=Form(), password=Form(), database=Form(), session_id=Form(None)
):
    try:
        graph = create_graph_database_connection(uri, userName, password, database)
        result = await asyncio.to_thread(
...
async def connect(uri=Form(), userName=Form(), password=Form(), database=Form()):
    try:
        graph = create_graph_database_connection(uri, userName, password, database)
        result = await asyncio.to_thread(
            connection_check_and_get_vector_dimensions, graph
        )
        josn_obj = {
            "api_name": "connect",
            "db_url": uri,
...
async def get_structured_schema(
    uri=Form(), userName=Form(), password=Form(), database=Form()
):
    try:
        graph = create_graph_database_connection(uri, userName, password, database)
