...
const ResultOverview: React.FunctionComponent<OverViewProps> = ({
  ...
  const nodeCount = (nodes: ExtendedNode[], label: string): number => {
    return [
      ...new Set(
        nodes?.filter((n) => n.labels?.includes(label)).map((i) => i.id)
      ),
    ].length;
  };
  ...
  const relationshipsSorted = relationships.sort((a, b) =>
  ...
  const relationshipCount = (
    ...
    return [
      ...new Set(
        relationships
          ?.filter((r) => r.caption?.includes(label))
          .map((i) => i.id)
      ),
    ].length;
  };
  ...
  const groupedAndSortedRelationships: ExtendedRelationship[] = Object.values(
    relationshipsSorted.reduce(
      (acc: { [key: string]: ExtendedRelationship }, relType: Relationship) => {
        const key = relType.caption || "";
        if (!acc[key]) {
          acc[key] = { ...relType, count: 0 };
        }
        (acc[key] as { count: number }).count += relationshipCount(
