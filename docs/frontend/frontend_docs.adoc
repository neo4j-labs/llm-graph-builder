...
*** __PART_OF__ - relation from Document node to all chunk nodes 
*** __FIRST_CHUNK__ - relation from document node to first chunk node
*** __NEXT_CHUNK__ - relation from a chunk pointing to next chunk of the document.
*** __HAS_ENTITY__ - relation between chunk node and entities extracted from LLM.
